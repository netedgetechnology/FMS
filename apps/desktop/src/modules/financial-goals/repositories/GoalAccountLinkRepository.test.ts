...
import { GoalAccountLinkRepository } from "./GoalAccountLinkRepository";
...
describe("GoalAccountLinkRepository", () => {
    beforeEach(() => {
    ...
    it("creates a link defaulting isActive to true", async () => {
        const repo = new GoalAccountLinkRepository();
    ...
    it("lists multiple links for the same goal, ordered by created_at", async () => {
        ...
        expect(list.map(l => l.accountId)).toEqual([
            "acc-1",
            "acc-2",
        ]);
    });
